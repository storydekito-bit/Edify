...
import { activeRewardUnlocks, hasAnyPremium, hasRewardItem, loadPremiumAccess, rewardItemNames, rewardTimeRemaining, type PremiumAccess } from '../../lib/premium';
import type { Clip, MediaAsset, PanelId, ProjectDocument, Toast } from '../../types/edify';
...
const desktopOnlyAiActions = new Set([
  'voice',
  'silence',
  'background',
  'upscale',
...
const desktopOnlySoundNames = new Set([
  ...soundItems.filter((_, index) => index % 10 < 7).map((sound) => sound.name),
  ...premiumSoundItems.map((sound) => sound.name)
]);

function lockItemsByRatio(items: string[], lockedRatio = 0.8) {
  if (items.length <= 1) return new Set<string>();
  const unlockedCount = Math.max(1, Math.ceil(items.length * (1 - lockedRatio)));
  return new Set(items.slice(unlockedCount));
}
...
export function LeftSidebar({
  ...
  const desktopLockedEffects = useMemo(() => isBrowserMode ? lockItemsByRatio(effectPresets) : new Set<string>(), [isBrowserMode]);
  const desktopLockedTransitions = useMemo(() => isBrowserMode ? lockItemsByRatio(transitionPresets) : new Set<string>(), [isBrowserMode]);
  const desktopLockedTexts = useMemo(() => isBrowserMode ? lockItemsByRatio(textPresets) : new Set<string>(), [isBrowserMode]);
  const desktopLockedTextAnimations = useMemo(() => isBrowserMode ? lockItemsByRatio(textAnimationPresets) : new Set<string>(), [isBrowserMode]);
  const desktopLockedFilters = useMemo(() => isBrowserMode ? lockItemsByRatio(filterPresets) : new Set<string>(), [isBrowserMode]);
  const desktopLockedStickers = useMemo(() => isBrowserMode ? lockItemsByRatio(stickerItems) : new Set<string>(), [isBrowserMode]);
  const desktopLockedPremiumEffects = useMemo(() => isBrowserMode ? lockItemsByRatio(premiumEffectPresets.map((item) => item.name)) : new Set<string>(), [isBrowserMode]);
  const desktopLockedPremiumTransitions = useMemo(() => isBrowserMode ? lockItemsByRatio(premiumTransitionPresets.map((item) => item.name)) : new Set<string>(), [isBrowserMode]);
  const desktopLockedPremiumTexts = useMemo(() => isBrowserMode ? lockItemsByRatio(premiumTextPresets.map((item) => item.name)) : new Set<string>(), [isBrowserMode]);
  const desktopLockedPremiumTextAnimations = useMemo(() => isBrowserMode ? lockItemsByRatio(premiumTextAnimationPresets.map((item) => item.name)) : new Set<string>(), [isBrowserMode]);
  const desktopLockedPremiumFilters = useMemo(() => isBrowserMode ? lockItemsByRatio(premiumFilterPresets.map((item) => item.name)) : new Set<string>(), [isBrowserMode]);
  const desktopLockedPremiumCaptions = useMemo(() => isBrowserMode ? lockItemsByRatio(premiumCaptionPresets.map((item) => item.name)) : new Set<string>(), [isBrowserMode]);
  ...
  const openDesktopFeatureNotice = (title: string, detail: string) => {
    onDesktopFeature(title, detail);
  };

  const openDesktopPresetNotice = (category: string, name: string) => {
    openDesktopFeatureNotice(
      `${category} available on desktop`,
      `${name} is available in the Edify desktop application, where the fuller preset browser, timeline placement, and render workflow are enabled.`
    );
  };

  const isDesktopLockedFavoritePreset = (item: FavoritePreset) => {
    if (!isBrowserMode) return false;
    switch (item.kind) {
      case 'effect':
        return desktopLockedEffects.has(item.name) || desktopLockedTextAnimations.has(item.name);
      case 'transition':
        return desktopLockedTransitions.has(item.name);
      case 'text':
        return desktopLockedTexts.has(item.name);
      case 'sticker':
        return desktopLockedStickers.has(item.name);
      case 'premium':
        return desktopLockedPremiumEffects.has(item.name);
      case 'premium-transition':
        return desktopLockedPremiumTransitions.has(item.name);
      case 'premium-text':
        return desktopLockedPremiumTexts.has(item.name);
      case 'premium-text-animation':
        return desktopLockedPremiumTextAnimations.has(item.name);
      case 'premium-filter':
        return desktopLockedPremiumFilters.has(item.name);
      case 'premium-caption':
        return desktopLockedPremiumCaptions.has(item.name);
      case 'filter':
        return desktopLockedFilters.has(item.name);
      default:
        return false;
    }
  };
  ...
  const toggleAutoCaptions = () => {
    if (isBrowserMode) {
      openDesktopFeatureNotice(
        ...
        'Live microphone captions, direct speech capture, and real-time voice transcription are available in the Edify desktop application.'
      );
      return;
    }
  ...
  const runAiTool = (tool: typeof aiTools[number]) => {
    if (isBrowserMode && desktopOnlyAiActions.has(tool.action)) {
      openDesktopFeatureNotice(
        `${tool.name} is desktop-only`,
        `${tool.name} is available in the Edify desktop application, where the heavier AI helpers, deeper processing chain, and advanced project workflows are enabled.`
      );
      return;
    }
  ...
  const addSoundToTimeline = (sound: typeof soundItems[number]) => {
    if (isBrowserMode && desktopOnlySoundNames.has(sound.name)) {
      openDesktopFeatureNotice(
        'Desktop sound pack',
        `${sound.name} is currently available in the Edify desktop application, together with the extended sound browser, richer preview workflow, and direct desktop audio handling.`
      );
      return;
    }
  ...
  const addPremiumSoundToTimeline = (sound: typeof premiumSoundItems[number]) => {
    if (isBrowserMode) {
      openDesktopFeatureNotice(
        'Desktop premium sound pack',
        `${sound.name} is available in the Edify desktop application, where premium sound packs, stronger previewing, and desktop audio workflows are fully enabled.`
      );
      return;
    }
  ...
  const previewSound = (sound: typeof soundItems[number]) => {
    if (isBrowserMode && desktopOnlySoundNames.has(sound.name)) {
      openDesktopFeatureNotice(
        'Desktop sound preview',
        `${sound.name} uses the fuller desktop preview player, waveform tools, and sound placement workflow available in the Edify desktop application.`
      );
      return;
    }
    const asset = createGeneratedSoundAsset(sound);
  ...
  return (
    <aside className="left-sidebar" onClick={() => setAssetMenu(null)}>
      <nav className="rail">
        ...
        <button
          className={`rail-account-button ${authUserName ? 'is-connected' : ''}`}
          title={authUserName ? `Connected as ${authUserName}` : 'Sign in or create an account'}
          onClick={onOpenAccount}
        >
          <CircleUserRound size={18} />
          <span>{authUserName ? 'Account on' : 'Account'}</span>
          <small>{authUserName ? 'Connected' : 'Sign in'}</small>
        </button>
      </nav>
      ...
      <section className="asset-panel">
        ...
        {activePanel === 'stickers' && (
          <div className="feature-stack">
            ...
            <PresetGrid
              ...
              lockedItems={desktopLockedStickers}
              onLockedSelect={(name) => openDesktopPresetNotice('Sticker', name)}
            ...
            <div className="preset-grid">
              {favoritePresets.map((item) => (
                <article
                  ...
                  draggable={!isDesktopLockedFavoritePreset(item)}
                  onClick={() => {
                    if (isDesktopLockedFavoritePreset(item)) {
                      openDesktopPresetNotice('Preset', item.name);
                      return;
                    }
        ...
        {activePanel === 'quick' && (
          <div className="feature-stack">
            <div className="sidebar-hero-card">
              ...
              <p>One-click edit recipes that add text layers and stack effects on your selected clip.</p>
            </div>
            ...
            <div className="sound-list">
              {filteredPremiumSounds.map((sound) => (
                <div
                  ...
                  key={sound.name}
                  draggable={hasAnyPremium(premiumAccess) && !isBrowserMode}
                  onDragStart={(event) => {
                    if (!hasAnyPremium(premiumAccess)) return;
                    if (isBrowserMode) {
                      event.preventDefault();
                      openDesktopFeatureNotice(
                        'Desktop premium sound drag',
                        `${sound.name} can be dragged from the full sound library in the Edify desktop application.`
                      );
                      return;
                    }
              ...
              {filteredSounds.map((sound) => (
                <div
                  className="sound-row"
                  key={sound.name}
                  draggable={!isBrowserMode || !desktopOnlySoundNames.has(sound.name)}
                  onDragStart={(event) => {
                    if (isBrowserMode && desktopOnlySoundNames.has(sound.name)) {
                      event.preventDefault();
                      openDesktopFeatureNotice(
                        'Desktop sound drag',
                        `${sound.name} can be dragged from the fuller sound library in the Edify desktop application.`
                      );
                      return;
                    }
                    event.dataTransfer.setData('application/x-edify-sound', sound.name);
        ...
        {activePanel === 'text' && (
          <>
            ...
            <PremiumPresetGrid
              ...
              desktopLockedItems={desktopLockedPremiumTexts}
              onDesktopLocked={(item) => openDesktopPresetNotice('Text preset', item.name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
              presetKind="text"
            ...
            <PremiumPresetGrid
              ...
              desktopLockedItems={desktopLockedPremiumTextAnimations}
              onDesktopLocked={(item) => openDesktopPresetNotice('Text animation', item.name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
              presetKind="effect"
            ...
            <PresetGrid
              ...
              dragKind="text"
              onSelect={onAddText}
              lockedItems={desktopLockedTexts}
              onLockedSelect={(name) => openDesktopPresetNotice('Text preset', name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
            />
            <div className="premium-inline-heading free-inline-heading">
            ...
            <PresetGrid
              ...
              dragKind="effect"
              onSelect={onAddEffect}
              lockedItems={desktopLockedTextAnimations}
              onLockedSelect={(name) => openDesktopPresetNotice('Text animation', name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
            />
          </>
        ...
        {activePanel === 'effects' && (
          <>
            ...
            <PremiumPresetGrid
              ...
              desktopLockedItems={desktopLockedPremiumEffects}
              onDesktopLocked={(item) => openDesktopPresetNotice('Effect', item.name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
              presetKind="effect"
            ...
            <PresetGrid
              ...
              dragKind="effect"
              onSelect={onAddEffect}
              lockedItems={desktopLockedEffects}
              onLockedSelect={(name) => openDesktopPresetNotice('Effect', name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
            />
          </>
        ...
        {activePanel === 'transitions' && (
          <>
            ...
            <PremiumPresetGrid
              ...
              desktopLockedItems={desktopLockedPremiumTransitions}
              onDesktopLocked={(item) => openDesktopPresetNotice('Transition', item.name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
            ...
            <PresetGrid
              ...
              dragKind="transition"
              onSelect={onAddTransition}
              lockedItems={desktopLockedTransitions}
              onLockedSelect={(name) => openDesktopPresetNotice('Transition', name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
            />
          </>
        ...
        {activePanel === 'filters' && (
          <>
            ...
            <PremiumPresetGrid
              ...
              desktopLockedItems={desktopLockedPremiumFilters}
              onDesktopLocked={(item) => openDesktopPresetNotice('Filter', item.name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
              presetKind="effect"
            ...
            <PresetGrid
              ...
              dragKind="filter"
              onSelect={onAddEffect}
              lockedItems={desktopLockedFilters}
              onLockedSelect={(name) => openDesktopPresetNotice('Filter', name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
            />
          </>
        ...
        {activePanel === 'captions' && (
          <div className="feature-stack">
            <h3>Captions</h3>
            <p>Live mic captions use Chromium speech when available. Smart captions always creates timeline subtitle clips.</p>
            <div className="premium-inline-heading">
              <Crown size={15} />
            ...
            <PremiumPresetGrid
              ...
              desktopLockedItems={desktopLockedPremiumCaptions}
              onDesktopLocked={(item) => openDesktopPresetNotice('Caption style', item.name)}
              favoriteSet={favoriteSet}
              onToggleFavorite={toggleFavorite}
              presetKind="text"
          ...
          <div className="feature-stack premium-panel">
            <div className="premium-hero-card">
              ...
              <div className="premium-status-pill">
                <Crown size={13} />
                {premiumAccess.all ? 'Studio Max active' : premiumAccess.packs.length > 0 ? `${premiumAccess.packs.length} pack active` : hasSponsoredReward ? `Sponsored trial ${formatRewardTime(sponsoredRewardRemaining)}` : 'Free plan active'}
              </div>
            </div>
            ...
            <div className="sponsored-trial-card">
              <div>
                ...
                <strong>{hasSponsoredReward ? `${sponsoredRewardItems.length} premium items unlocked` : 'Watch Edify sponsor reel'}</strong>
                <small>{hasSponsoredReward ? `Expires in ${formatRewardTime(sponsoredRewardRemaining)}. Premium plans still unlock everything permanently.` : '30 seconds unlocks 5 random base premium items for 24 hours.'}</small>
              </div>
              <button className="secondary-button" onClick={() => setShowSponsoredUnlock(true)}>
                {hasSponsoredReward ? 'Refresh trial' : 'Start 30s reel'}
              </button>
            </div>
            {hasSponsoredReward && (
        ...
        {activePanel === 'marketplace' && (
          <div className="feature-stack">
            <div className="sidebar-hero-card market-hero-card">
              ...
              <p>Premium packs are shown up front; free packs install instantly for the current project.</p>
            </div>
        ...
        {activePanel === 'color' && (
          <div className="feature-stack">
            <div className="sidebar-hero-card color-hero-card">
              <Palette size={18} />
              <h3>Color Studio</h3>
              <p>Fast grading tools, creator looks, labels, and export-safe color polish.</p>
            </div>
...
function PresetGrid({
  ...
  onLockedSelect?: (item: string) => void;
  ...
  return (
    <div className="preset-grid">
      {items.map((item) => {
        const locked = lockedItems?.has(item) ?? false;
        return (
        <article
          className={`preset-tile preset-${dragKind} ${locked ? 'premium-locked' : ''} preset-${slugify(item)}`}
          key={item}
          draggable={!locked}
          onClick={() => (locked ? onLockedSelect?.(item) : onSelect(item))}
          onDragStart={(event) => {
            if (locked) {
              event.preventDefault();
              return;
...
function PremiumPresetGrid({
  ...
  desktopLockedItems?: Set<string>;
  onDesktopLocked?: (item: PremiumEffectPreset) => void;
  favoriteSet: Set<string>;
  onToggleFavorite: (kind: FavoritePreset['kind'], name: string) => void;
  ...
  return (
    <div className="preset-grid premium-preset-grid">
      {items.map((item) => {
        const desktopLocked = desktopLockedItems?.has(item.name) ?? false;
        const unlocked = canUse(item) && !desktopLocked;
        return (
          <article
            className={`preset-tile preset-${presetKind} premium-preset ${unlocked ? 'premium-unlocked' : 'premium-locked'} preset-${slugify(item.name)}`}
            key={item.name}
            draggable={unlocked}
            onClick={() => (desktopLocked ? onDesktopLocked?.(item) : unlocked ? onSelect(item.name) : onLocked(item))}
            onDragStart={(event) => {
              if (!unlocked) {
                event.preventDefault();
                return;
...
function SoundWave({ name }: { name: string }) {
  return (
    <span className="sound-wave" aria-hidden="true">
      {createWaveformForName(name, 10).map((value, index) => (
        <i key={index} style={{ height: `${Math.max(8, value * 0.32)}px` }} />
      ))}
    </span>
  );
}
