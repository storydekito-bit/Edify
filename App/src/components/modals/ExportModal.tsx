import { CheckCircle2, Crown, Download, FileVideo, FolderOpen, Loader2, ShieldCheck, X } from 'lucide-react';
...
import type { DesktopAccountUser, ProjectDocument, Toast } from '../../types/edify';
...
type ExportMode = 'Publish master' | 'Client review';
...
export function ExportModal({ project, accountUser, onOpenAccount, onClose, pushToast }: ExportModalProps) {
  ...
  const [exportMode, setExportMode] = useState<ExportMode>('Publish master');
  ...
  const recommendedTargets = useMemo(() => {
    if (project.settings.aspectRatio === '9:16') return ['TikTok / Shorts', 'Instagram Reels', 'Snap vertical ads'];
    if (project.settings.aspectRatio === '1:1' || project.settings.aspectRatio === '4:5') return ['Instagram feed', 'Product ads', 'Social previews'];
    if (project.settings.aspectRatio === '21:9') return ['Trailer showcase', 'Cinema teaser', 'Premium portfolio'];
    return ['YouTube 1080p', 'Discord preview', 'Client delivery'];
  }, [project.settings.aspectRatio]);
  const reviewOverlayLabel = exportMode === 'Client review' ? 'Client watermark and review notes' : 'Publish-ready output';
  ...
  useEffect(() => {
    return edifyApi.onExportProgress((payload: unknown) => {
      const next = payload as ExportProgress & { type?: string };
      if (next.type === 'app-update' || !next.jobId) return;
      setProgress(next);
  ...
  const startExport = async () => {
    ...
    const result = await edifyApi.startExport({
      ...
      burnCaptions,
      estimatedSizeMb,
      watermark: exportMode === 'Client review' || !hasPremiumPlan,
      project
    });
    setIsStarting(false);
    if (!result?.canceled) {
        ...
        <div className="export-form">
          ...
          <section className="export-card">
            ...
            <label>
              ...
              <select value={exportMode} onChange={(event) => setExportMode(event.target.value as ExportMode)}>
                <option>Publish master</option>
                <option>Client review</option>
              </select>
            </label>
            <div className="export-summary-strip">
              <span>{reviewOverlayLabel}</span>
              <strong>{exportMode === 'Client review' ? 'Safe share' : 'Final output'}</strong>
            </div>
            <div className="export-target-list">
              {recommendedTargets.map((target) => (
                <span className="review-pill pill-ok" key={target}>{target}</span>
              ))}
            </div>
          </section>
          ...
          <section className="export-card export-summary-card">
            ...
            <dl>
              ...
              <div><dt>Delivery</dt><dd>{exportMode}</dd></div>
              <div><dt>Watermark</dt><dd>{exportMode === 'Client review' ? 'Client preview' : hasPremiumPlan ? 'No' : 'Free badge'}</dd></div>
            </dl>
          </section>

          <section className="export-card export-engine-card">
            <h3><ShieldCheck size={15} /> Render engine</h3>
            <div className="engine-feature-list">
              <div>
                <strong>FFmpeg desktop path</strong>
                <span>MP4-first export, mixed audio, text burn-in, and real file output through the desktop render engine.</span>
              </div>
              <div>
                <strong>Review and watermark logic</strong>
                <span>Client review mode keeps a safe preview path, while Premium removes the standard publish watermark.</span>
              </div>
              <div>
                <strong>Future-ready delivery</strong>
                <span>MOV, WebM, review exports, and richer preset routing can all hang off this same engine path cleanly.</span>
              </div>
            </div>
          </section>
        </div>
