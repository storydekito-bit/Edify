...
function motionEasingForClip(clip: Clip) {
  const easingEffect = clip.effects.find((effect) => effect.enabled && /motion easing:/i.test(effect.name));
  const key = easingEffect?.name.toLowerCase() ?? '';
  if (key.includes('linear')) return easeLinear;
  if (key.includes('smooth')) return easeInOut;
  if (key.includes('elastic')) return easeElastic;
  return easeOut;
}

function interpolateKeyframedValue(clip: Clip, property: Clip['keyframes'][number]['property'], baseValue: number, playhead: number) {
  const frames = clip.keyframes
    .filter((frame) => frame.property === property)
    .sort((left, right) => left.time - right.time);
  if (frames.length === 0) return baseValue;
  if (playhead <= frames[0].time) return frames[0].value;
  const last = frames[frames.length - 1];
  if (playhead >= last.time) return last.value;
  const nextIndex = frames.findIndex((frame) => frame.time >= playhead);
  if (nextIndex <= 0) return frames[0].value;
  const previous = frames[nextIndex - 1];
  const next = frames[nextIndex];
  const span = Math.max(0.001, next.time - previous.time);
  const ratio = (playhead - previous.time) / span;
  const eased = motionEasingForClip(clip)(ratio);
  return previous.value + (next.value - previous.value) * eased;
}

function resolvedClipTransform(clip: Clip, playhead: number) {
  return {
    x: interpolateKeyframedValue(clip, 'x', clip.transform.x, playhead),
    y: interpolateKeyframedValue(clip, 'y', clip.transform.y, playhead),
    scale: interpolateKeyframedValue(clip, 'scale', clip.transform.scale, playhead),
    rotation: interpolateKeyframedValue(clip, 'rotation', clip.transform.rotation, playhead),
    opacity: interpolateKeyframedValue(clip, 'opacity', clip.transform.opacity, playhead)
  };
}
...
function clipGainWithKeyframes(clip: Clip, playhead: number, masterVolume = 1) {
  const local = Math.max(0, playhead - clip.start);
  const fadeIn = clip.audio.fadeIn > 0 ? clamp01(local / clip.audio.fadeIn) : 1;
  const fadeOutStart = Math.max(0, clip.duration - clip.audio.fadeOut);
  const fadeOut = clip.audio.fadeOut > 0 && local > fadeOutStart ? clamp01((clip.duration - local) / clip.audio.fadeOut) : 1;
  const baseVolume = interpolateKeyframedValue(clip, 'volume', clip.audio.volume, playhead);
  return Math.max(0, Math.min(1, baseVolume * fadeIn * fadeOut * masterVolume));
}
...
function PreviewMediaLayer({
  ...
  const currentTransform = useMemo(() => resolvedClipTransform(clip, playhead), [clip, playhead]);
  const activeScale = currentTransform.scale * activeEffects.extraScale;
  ...
  useEffect(() => {
    ...
    video.volume = clipGainWithKeyframes(clip, playhead, masterVolume);
  ...
  return (
    <div
      ...
      style={{
        transform: `translate(${currentTransform.x}px, ${currentTransform.y}px) scale(${activeScale}) rotate(${currentTransform.rotation}deg)${activeTransition.extraTransform}`,
        opacity: activeTransition.opacity ?? currentTransform.opacity,
...
export function PreviewPlayer({ editor }: { editor: EditorController }) {
  ...
  const transformClip =
    editor.selectedClip && (editor.selectedClip.kind === 'video' || editor.selectedClip.kind === 'image' || editor.selectedClip.kind === 'text')
      ...
      : topVisibleMediaClip
        ? topVisibleMediaClip
        : undefined;
  const transformBoxTransform = transformClip ? resolvedClipTransform(transformClip, editor.playhead) : { x: 0, y: 0, scale: 1, rotation: 0, opacity: 1 };
  ...
  useEffect(() => {
    ...
    audio.volume = clipGainWithKeyframes(activeAudioClip, editor.playhead, masterVolume);
      ...
      <div className="preview-stage">
        ...
        <div className="preview-frame" ref={frameRef} style={{ aspectRatio: frameAspect, width: frameWidth }}>
          ...
          {previewTextClips.map((textClip) => {
            ...
            const textTransform = resolvedClipTransform(textClip, editor.playhead);
            return (
              <div
