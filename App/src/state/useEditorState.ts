...
export function useEditorState(initialProject: ProjectDocument) {
  ...
  const rippleDeleteSelectedClip = useCallback(() => {
    ...
    commit((project) => ({
      ...project,
      tracks: project.tracks.map((track) => {
        const removedClip = track.clips.find((clip) => clip.id === state.selectedClipId);
        if (!removedClip) return track;
        const shift = removedClip.duration;
        return {
          ...track,
          clips: track.clips
            .filter((clip) => clip.id !== state.selectedClipId)
            .map((clip) =>
              clip.start >= removedClip.start + removedClip.duration - 0.001
                ? { ...clip, start: Math.max(0, Math.round((clip.start - shift) * 30) / 30) }
                : clip
            )
        };
      })
  ...
  const applySelectedMotionToAllClips = useCallback(() => {
    if (!state.selectedClipId) return;
    commit((project) => {
      const source = findClip(project, state.selectedClipId);
      if (!source) return project;
      const sourceKind = source.kind === 'image' ? 'video' : source.kind;
      const motionEffects = source.effects
        .filter((effect) => /motion easing|speed|motion blur|shake|zoom punch|glow|rgb shift/i.test(`${effect.name} ${effect.kind}`))
        .map((effect) => ({ ...effect, id: createId('fx') }));
      return {
        ...project,
        tracks: project.tracks.map((track) => ({
          ...track,
          clips: track.clips.map((clip) => {
            ...
            return {
              ...
              keyframes: source.keyframes.map((frame) => ({ ...frame, id: createId('keyframe') })),
              effects: [
                ...clip.effects.filter((effect) => !/motion easing|speed|motion blur|shake|zoom punch|glow|rgb shift/i.test(`${effect.name} ${effect.kind}`)),
                ...motionEffects.map((effect) => ({ ...effect, id: createId('fx') }))
              ]
            };
          })
        }))
